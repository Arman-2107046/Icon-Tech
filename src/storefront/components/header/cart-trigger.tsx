...
function Bag({ count }: { count?: number }) {
  return (
    <Link href="/cart" className="relative inline-flex size-11 items-center justify-center rounded-sf-full text-ink transition-colors hover:bg-neutral-100" aria-label={count ? `Cart, ${count} item${count === 1 ? "" : "s"}` : "Cart"} data-testid="cart-trigger">
